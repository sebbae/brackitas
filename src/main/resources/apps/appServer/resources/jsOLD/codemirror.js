...
var CodeMirror = (function() {
	...
	function addLineNumberDiv(container) {
		var nums = document.createElement("DIV"), scroller = document
				.createElement("DIV");
	...
	function frameHTML(options) {
		...
		forEach(options.basefiles.concat(options.parserfile), function(file) {
			...
			html.push("<script type=\"text/javascript\" src=\"" + file + "\"><"
					+ "/script>");
		});
		html
	...
	function CodeMirror(place, options) {
		...
		div.style.height = (options.height == "dynamic") ? options.minHeight
				+ "px" : options.height;
		...
		if (options.lineNumbers)
			this.lineNumbers = addLineNumberDiv(div);
	...
	CodeMirror.prototype = {
		...
		focus : function() {
			...
			if (this.editor.selectionSnapshot) // IE hack
				this.win.select.setBookmark(this.win.document.body,
		...
		setStylesheet : function(names) {
			...
			for ( var y = 0; y < names.length; y++) {
				...
				if (!(name in matchedNames)) {
					...
					link.type = "text/css";
					link.href = name;
					this.win.document.getElementsByTagName('head')[0]
							.appendChild(link);
				}
			}
		},
		...
		activateLineNumbers : function() {
			var frame = this.frame, win = frame.contentWindow, doc = win.document, body = doc.body, nums = this.lineNumbers, scroller = nums.firstChild, self = this;
			var barWidth = null;
			...
			function ensureEnoughLineNumbers(fill) {
				...
				if (lineHeight == 0)
					return;
				var targetHeight = 50 + Math.max(body.offsetHeight, Math.max(
						frame.offsetHeight, body.scrollHeight || 0)), lastNumber = Math
						.ceil(targetHeight / lineHeight);
				for ( var i = scroller.childNodes.length; i <= lastNumber; i++) {
					var div = document.createElement("DIV");
					div.appendChild(document
							.createTextNode(fill ? String(i + 1) : "\u00a0"));
			...
			function nonWrapping() {
				...
				var onScroll = win.addEventHandler(win, "scroll", doScroll,
						true), onResize = win.addEventHandler(win, "resize",
						update, true);
				clear = function() {
					onScroll();
					onResize();
			...
			function wrapping() {
				...
				function setNum(n, node) {
					...
					if (!lineNum)
						lineNum = scroller.appendChild(document
								.createElement("DIV"));
					if (styleNums)
				...
				function work() {
					if (!scroller.parentNode
							|| scroller.parentNode != self.lineNumbers)
						return;

					var endTime = new Date().getTime()
							+ self.options.lineNumberTime;
					while (node) {
						...
						if (new Date().getTime() > endTime) {
							commitChanges();
							pending = setTimeout(work,
									self.options.lineNumberDelay);
							return;
						}
					}
					while (lineNum)
			...
			(this.options.textWrapping || this.options.styleNumbers ? wrapping
					: nonWrapping)();
		},

		setDynamicHeight : function() {
			var self = this, activity = self.options.cursorActivity, win = self.win, body = win.document.body, lineHeight = null, timeout = null, vmargin = 2 * self.frame.offsetTop;
					...
					if (win.isSpan(span) && span.offsetHeight) {
						...
						if (!sawBR)
							vmargin = 2 * (self.frame.offsetTop
									+ span.offsetTop + body.offsetTop + (internetExplorer ? 10
									: 0));
						break;
					}
				if (lineHeight)
					self.wrapping.style.height = Math.max(vmargin + lineHeight
							* (body.getElementsByTagName("BR").length + 1),
