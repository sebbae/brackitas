...
var CodeMirror = (function() {
	...
	function CodeMirror(place, givenOptions) {
		...
		function handleKeyBinding(e) {
			...
			if (typeof bound != "string")
				bound(instance);
			else if (commands.propertyIsEnumerable(bound))
		...
		function onKeyDown(e) {
			...
			if (window.opera) {
				...
				if (!handled && (mac ? e.metaKey : e.ctrlKey)
						&& e.keyCode == 88)
					replaceSelection("");
			}
		}
					...
					.iter(
							...
							function(line) {
								if (line.hidden) {
									...
									html
											.push(
													(marker && marker.style ? '<pre class="' + marker.style + '">'
															: "<pre>"), text);
			...
			if (unit == "char")
				...
				for (;;) {
					if (dir < 0)
						if (!moveOnce())
							break;
					if (/\w/.test(lineObj.text.charAt(ch)))
						sawWord = true;
					else if (sawWord) {
						if (dir < 0) {
							dir = 1;
	...
	Line.prototype = {
		...
		getHTML : function(sfrom, sto, includePre, tabText, endAt) {
			...
			if (includePre)
				html
						.push(this.className ? '<pre class="' + this.className + '">'
								: "<pre>");
